...
function addBall(pitch, pitchType) {
  ...
  const spinAxis = pitch.spin_axis || 0;
  const radiansPerSec = (spinRate * 2 * Math.PI) / 60;
  const angleRad = THREE.MathUtils.degToRad(spinAxis);
  const axis = new THREE.Vector3(Math.cos(angleRad), Math.sin(angleRad), 0).normalize();
...
function animate() {
  ...
  if (playing) {
    for (let ball of balls) {
      const { t0, release, velocity, accel, spinAxis, spinRate } = ball.userData;
      const t = now - t0;
      ...
      if (spinAxis && spinRate) {
        ball.rotateOnAxis(spinAxis, spinRate * delta);
      }
    }
  }
